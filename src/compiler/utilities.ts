...
export function getResolvedModule(
    ...
    return sourceFile?.resolvedModules?.get(moduleNameText, mode)
        ?.resolvedModule;
}
...
export function getResolvedTypeReferenceDirective(
    ...
    return sourceFile?.resolvedTypeReferenceDirectiveNames?.get(
        typeReferenceDirectiveName,
        mode
    )?.resolvedTypeReferenceDirective;
}
...
export function moduleResolutionIsEqualTo(
    ...
    return (
        ...
        (!!oldResolution.resolvedModule &&
            ...
            oldResolution.resolvedModule.extension ===
                newResolution.resolvedModule.extension &&
            oldResolution.resolvedModule.resolvedFileName ===
                newResolution.resolvedModule.resolvedFileName &&
            oldResolution.resolvedModule.originalPath ===
                newResolution.resolvedModule.originalPath &&
            packageIdIsEqual(
                oldResolution.resolvedModule.packageId,
                newResolution.resolvedModule.packageId
            ))
    );
}
...
export function isGrammarError(parent: Node, child: Node | NodeArray<Node>) {
    ...
    if (isPropertyDeclaration(parent))
        return (
            child === parent.questionToken &&
            isAutoAccessorPropertyDeclaration(parent)
        );
    if (isPropertyAssignment(parent))
        return (
            child === parent.modifiers ||
            child === parent.questionToken ||
            child === parent.exclamationToken ||
            isGrammarErrorElement(parent.modifiers, child, isModifierLike)
        );
    if (isShorthandPropertyAssignment(parent))
        return (
            child === parent.equalsToken ||
            child === parent.modifiers ||
            child === parent.questionToken ||
            child === parent.exclamationToken ||
            isGrammarErrorElement(parent.modifiers, child, isModifierLike)
    ...
    if (isConstructorDeclaration(parent))
        return (
            child === parent.typeParameters ||
            child === parent.type ||
            isGrammarErrorElement(
...
export function isRecognizedTripleSlashComment(
    ...
    if (
        ...
        return fullTripleSlashReferencePathRegEx.test(textSubStr) ||
            ...
            fullTripleSlashReferenceTypeReferenceDirectiveRegEx.test(
                textSubStr
            ) ||
...
export function getNonDecoratorTokenPosOfNode(
    ...
    const lastDecorator =
        !nodeIsMissing(node) && canHaveModifiers(node)
            ? findLast(node.modifiers, isDecorator)
            : undefined;
    if (!lastDecorator) {
...
/** @internal */
export type ScriptTargetFeatures = ReadonlyMap<
    string,
...
export function getScriptTargetFeatures(): ScriptTargetFeatures {
    return new Map(
        Object.entries({
            ...
            AsyncIterable: new Map(
                Object.entries({
                    es2018: emptyArray,
                })
            ),
            AsyncIterableIterator: new Map(
                Object.entries({
                    es2018: emptyArray,
                })
            ),
            AsyncGenerator: new Map(
                Object.entries({
                    es2018: emptyArray,
                })
            ),
            AsyncGeneratorFunction: new Map(
                Object.entries({
                    es2018: emptyArray,
                })
            ),
            ...
            ArrayConstructor: new Map(
                Object.entries({
                    es2015: ["from", "of"],
                })
            ),
            ObjectConstructor: new Map(
                Object.entries({
                    es2015: [
                        "assign",
            ...
            RegExpMatchArray: new Map(
                Object.entries({
                    es2018: ["groups"],
                })
            ),
            RegExpExecArray: new Map(
                Object.entries({
                    es2018: ["groups"],
                })
            ),
    ...
    switch (node.kind) {
        case SyntaxKind.StringLiteral: {
            ...
            if ((node as StringLiteral).singleQuote) {
                return (
                    "'" +
                    escapeText(node.text, CharacterCodes.singleQuote) +
                    "'"
                );
            } else {
                return (
                    '"' +
                    escapeText(node.text, CharacterCodes.doubleQuote) +
                    '"'
                );
            }
        }
...
/** @internal */
export function isAnyImportOrBareOrAccessedRequire(
    node: Node
): node is AnyImportOrBareOrAccessedRequire {
    return (
...
export function entityNameToString(
    ...
    switch (name.kind) {
        ...
        case SyntaxKind.PrivateIdentifier:
        case SyntaxKind.Identifier:
            return getFullWidth(name) === 0
                ? idText(name)
                : getTextOfNode(name);
        case SyntaxKind.QualifiedName:
            return (
                entityNameToString(name.left) +
                "." +
                entityNameToString(name.right)
            );
        ...
        case SyntaxKind.JsxNamespacedName:
            return (
                entityNameToString(name.namespace) +
                ":" +
                entityNameToString(name.name)
            );
        default:
...
export function getErrorSpanForNode(
    ...
    switch (node.kind) {
        ...
        case SyntaxKind.YieldExpression: {
            const pos = skipTrivia(
                sourceFile.text,
                (node as ReturnStatement | YieldExpression).pos
            );
            return getSpanOfTokenAtPosition(sourceFile, pos);
        }
        case SyntaxKind.SatisfiesExpression: {
            const pos = skipTrivia(
                sourceFile.text,
                (node as SatisfiesExpression).expression.end
            );
            return getSpanOfTokenAtPosition(sourceFile, pos);
        }
        case SyntaxKind.JSDocSatisfiesTag: {
            const pos = skipTrivia(
                sourceFile.text,
                (node as JSDocSatisfiesTag).tagName.pos
            );
            return getSpanOfTokenAtPosition(sourceFile, pos);
        }
    }
...
export function isPrologueDirective(node: Node): node is PrologueDirective {
    return (
        ...
        (node as ExpressionStatement).expression.kind ===
...
const fullTripleSlashLibReferenceRegEx =
    /^(\/\/\/\s*<reference\s+lib\s*=\s*)(('[^']*')|("[^"]*")).*?\/>/;
/** @internal */
    ...
    switch (node.kind) {
        ...
        case SyntaxKind.ThisKeyword: {
            ...
            switch (parent.kind) {
                ...
                case SyntaxKind.TypeParameter:
                    return (
                        node === (parent as TypeParameterDeclaration).constraint
                    );
                case SyntaxKind.JSDocTemplateTag:
                ...
                case SyntaxKind.TaggedTemplateExpression:
                    return contains(
                        (parent as CallExpression | TaggedTemplateExpression)
                            .typeArguments,
                        node
                    );
            }
    ...
    return (
        (isVariableDeclaration(node)
            ...
            : isPropertySignature(node) &&
              hasEffectiveReadonlyModifier(node)) ||
...
): SuperContainerOrFunctions | undefined;
...
export function nodeOrChildIsDecorated(
    ...
    return (
        nodeIsDecorated(useLegacyDecorators, node, parent!, grandparent!) ||
        childIsDecorated(useLegacyDecorators, node, parent!)
    );
}

/** @internal */
export function childIsDecorated(
    useLegacyDecorators: boolean,
    node: ClassDeclaration | ClassExpression
): boolean;
/** @internal */
    ...
    switch (node.kind) {
        case SyntaxKind.ClassDeclaration:
            return some((node as ClassDeclaration).members, (m) =>
                nodeOrChildIsDecorated(useLegacyDecorators, m, node, parent!)
            );
        case SyntaxKind.ClassExpression:
            return (
                !useLegacyDecorators &&
                some((node as ClassExpression).members, (m) =>
        ...
        case SyntaxKind.Constructor:
            return some((node as FunctionLikeDeclaration).parameters, (p) =>
                nodeIsDecorated(useLegacyDecorators, p, node, parent!)
            );
        default:
            return false;
    }
...
/** @internal */
export function isBindingElementOfBareOrAccessedRequire(
    node: Node
): node is BindingElementOfBareOrAccessedRequire {
    return (
        ...
        if (
            (id.escapedText === "exports" ||
                (id.escapedText === "module" &&
                    getElementOrPropertyAccessName(nextToLast) ===
                        "exports")) &&
...
/** @internal */
export interface PrototypePropertyAssignment
...
export function isPrototypePropertyAssignment(
    node: Node
): node is PrototypePropertyAssignment {
    return (
        isBinaryExpression(node) &&
        getAssignmentDeclarationKind(node) ===
...
export function isSpecialPropertyDeclaration(
    ...
    return (
        ...
        (!isElementAccessExpression(expr) ||
            isLiteralLikeElementAccess(expr)) &&
...
export function tryGetModuleSpecifierFromDeclaration(
    ...
    switch (node.kind) {
        ...
        case SyntaxKind.BindingElement:
            return findAncestor(
                node.initializer,
                (node): node is RequireOrImportCall =>
                    isRequireCall(
                        node,
                        /*requireStringLiteralLikeArgument*/ true
                    )
        ...
        case SyntaxKind.ImportEqualsDeclaration:
            return tryCast(
                tryCast(node.moduleReference, isExternalModuleReference)
                    ?.expression,
                isStringLiteralLike
            );
...
export function getNamespaceDeclarationNode(
    ...
    switch (node.kind) {
        ...
        case SyntaxKind.ExportDeclaration:
            return (
                node.exportClause &&
                tryCast(node.exportClause, isNamespaceExport)
            );
        default:
            return Debug.assertNever(node);
    }
    ...
    while (node && node.parent) {
        ...
        if (node.kind === SyntaxKind.Parameter) {
            result = addRange(
                ...
                (noCache
                    ? getJSDocParameterTagsNoCache
                    : getJSDocParameterTags)(node as ParameterDeclaration)
            );
            break;
        }
...
export function getHostSignatureFromJSDoc(
    ...
    if (host) {
        return isPropertySignature(host) &&
            host.type &&
...
export function getTypeParameterFromJsDoc(
    ...
    return (
        typeParameters &&
        find(typeParameters, (p) => p.name.escapedText === name)
    );
}
...
export function getAssignmentTargetKind(node: Node): AssignmentKind {
    ...
    while (true) {
        switch (parent.kind) {
            case SyntaxKind.BinaryExpression:
                const binaryOperator = (parent as BinaryExpression)
                    .operatorToken.kind;
...
export function isIdentifierName(node: Identifier): boolean {
    ...
    switch (parent.kind) {
        ...
        case SyntaxKind.PropertyAccessExpression:
            ...
            return (
                (parent as NamedDeclaration | PropertyAccessExpression).name ===
                node
            );
        case SyntaxKind.QualifiedName:
        ...
        case SyntaxKind.ImportSpecifier:
            ...
            return (
                (parent as BindingElement | ImportSpecifier).propertyName ===
                node
            );
        case SyntaxKind.ExportSpecifier:
...
export function isPunctuation(
    token: SyntaxKind
): token is PunctuationSyntaxKind {
    return (
        SyntaxKind.FirstPunctuation <= token &&
        token <= SyntaxKind.LastPunctuation
    );
}

/** @internal */
export function isKeywordOrPunctuation(
    token: SyntaxKind
): token is PunctuationOrKeywordSyntaxKind {
...
export function getTextOfIdentifierOrLiteral(
    node: PropertyNameLiteral | PrivateIdentifier
): string {
...
export function isAnonymousFunctionDefinition(
    ...
    return typeof cb === "function"
        ? cb(node as AnonymousFunctionDefinition)
        : true;
}
 ...
 * @internal
 */
export function isNamedEvaluationSource(
    node: Node
): node is NamedEvaluationSource {
    switch (node.kind) {
        case SyntaxKind.PropertyAssignment:
        ...
        case SyntaxKind.VariableDeclaration:
            return (
                isIdentifier((node as VariableDeclaration).name) &&
                !!(node as VariableDeclaration).initializer
            );
        case SyntaxKind.Parameter:
            return (
                isIdentifier((node as ParameterDeclaration).name) &&
                !!(node as VariableDeclaration).initializer &&
                !(node as BindingElement).dotDotDotToken
            );
        case SyntaxKind.BindingElement:
            return (
                isIdentifier((node as BindingElement).name) &&
                !!(node as VariableDeclaration).initializer &&
                !(node as BindingElement).dotDotDotToken
            );
        case SyntaxKind.PropertyDeclaration:
        ...
        !!(isNoSubstitutionTemplateLiteral(template)
            ...
            : template.head.templateFlags ||
              some(
                  ...
                  (span) => !!span.literal.templateFlags
              ))
    );
}
...
export function getEffectiveReturnTypeNode(
    ...
    return isJSDocSignature(node)
        ...
        : node.type ||
              (isInJSFile(node) ? getJSDocReturnType(node) : undefined);
}
    ...
    return (
        ...
        !(
            ...
            (tag.parent.tags!.some(isJSDocTypeAlias) ||
                tag.parent.tags!.some(isJSDocOverloadTag))
        )
    );
}
...
export function writeCommentRange(
    ...
    if (text.charCodeAt(commentPos + 1) === CharacterCodes.asterisk) {
        ...
        for (
            let pos = commentPos,
                currentLine = firstCommentLineAndCharacter.line;
            pos < commentEnd;
            currentLine++
        ) {
            const nextLineStart =
            ...
            if (pos !== commentPos) {
                ...
                if (spacesToEmit > 0) {
                    let numberOfSingleSpacesToEmit =
                        spacesToEmit % getIndentSize();
...
function getModifierFlagsWorker(
    ...
    if (!(node.modifierFlagsCache & ModifierFlags.HasComputedFlags)) {
        node.modifierFlagsCache =
            ...
            ModifierFlags.HasComputedFlags;
    }
...
export function getSyntacticModifierFlagsNoCache(node: Node): ModifierFlags {
    let flags = canHaveModifiers(node)
        ? modifiersToFlags(node.modifiers)
        : ModifierFlags.None;
    if (
        node.flags & NodeFlags.NestedNamespace ||
        (node.kind === SyntaxKind.Identifier &&
            node.flags & NodeFlags.IdentifierIsInJSDocNamespace)
    ) {
...
export function tryGetPropertyAccessOrIdentifierToString(
    ...
    if (isPropertyAccessExpression(expr)) {
        const baseStr = tryGetPropertyAccessOrIdentifierToString(
            expr.expression
        );
...
export function convertToBase64(input: string): string {
    ...
    while (i < length) {
        ...
        byte3 =
            ((charCodes[i + 1] & 0b00001111) << 2) | (charCodes[i + 2] >> 6);
...
export function readJsonOrUndefined(
    ...
    const jsonText = isString(hostOrText)
        ? hostOrText
        : hostOrText.readFile(path);
...
export function moveRangePastDecorators(node: Node): TextRange {
    const lastDecorator = canHaveModifiers(node)
        ? findLast(node.modifiers, isDecorator)
        : undefined;
...
export function moveRangePastModifiers(node: Node): TextRange {
    ...
    const lastModifier = canHaveModifiers(node)
        ? lastOrUndefined(node.modifiers)
        : undefined;
    ...
    if (parseNode) {
        switch (parseNode.parent.kind) {
            ...
            case SyntaxKind.ModuleDeclaration:
                return (
                    parseNode ===
                    (parseNode.parent as EnumDeclaration | ModuleDeclaration)
                        .name
                );
        }
    }
...
/** @internal */
export function isInitializedVariable(
    node: Node
...
export function getCheckFlags(symbol: Symbol): CheckFlags {
    return symbol.flags & SymbolFlags.Transient
        ? (symbol as TransientSymbol).links.checkFlags
        : 0;
}
...
export function getDeclarationModifierFlagsFromSymbol(
    ...
    if (s.valueDeclaration) {
        const declaration =
            (isWrite &&
                s.declarations &&
                find(s.declarations, isSetAccessorDeclaration)) ||
            (s.flags & SymbolFlags.GetAccessor &&
...
export function forEachNameInAccessChainWalkingLeft<T>(
    ...
    if (
        isAccessExpression(name.parent) &&
        isRightSideOfAccessExpression(name)
    ) {
    ...
    function walkAccessExpression(access: AccessExpression): T | undefined {
        if (access.kind === SyntaxKind.PropertyAccessExpression) {
            ...
            if (
                isIdentifier(access.argumentExpression) ||
                isStringLiteralLike(access.argumentExpression)
            ) {
 ...
 * @internal
 */
export function addObjectAllocatorPatcher(
    fn: (objectAllocator: ObjectAllocator) => void
) {
...
export function setLocalizedDiagnosticMessages(
    messages: MapLike<string> | undefined
) {
...
export function maybeSetLocalizedDiagnosticMessages(
    getMessages: undefined | (() => MapLike<string> | undefined)
) {
...
function isFileForcedToBeModuleByFormat(file: SourceFile): true | undefined {
    ...
    return (file.impliedNodeFormat === ModuleKind.ESNext ||
        fileExtensionIsOneOf(file.fileName, [
            Extension.Cjs,
            Extension.Cts,
            Extension.Mjs,
            Extension.Mts,
...
export function getEmitModuleDetectionKind(options: CompilerOptions) {
    return (
        ...
        (getEmitModuleKind(options) === ModuleKind.Node16 ||
        getEmitModuleKind(options) === ModuleKind.NodeNext
...
export function importNameElisionDisabled(options: CompilerOptions) {
    return (
        ...
        (options.isolatedModules && options.preserveValueImports)
    );
}
...
/** @internal */
export function moduleResolutionSupportsPackageJsonExportsAndImports(
...
export interface SymlinkCache {
    ...
    getSymlinkedDirectories():
        | ReadonlyMap<Path, SymlinkedDirectory | false>
        | undefined;
    ...
    return {
        ...
        setSymlinkedDirectory: (symlink, real) => {
            ...
            if (!containsIgnoredPath(symlinkPath)) {
                ...
                (
                    symlinkedDirectories || (symlinkedDirectories = new Map())
                ).set(symlinkPath, real);
            }
        },
        setSymlinksFromResolutions(files, typeReferenceDirectives) {
            ...
            for (const file of files) {
                file.resolvedModules?.forEach((resolution) =>
                    processResolution(this, resolution.resolvedModule)
                );
                file.resolvedTypeReferenceDirectiveNames?.forEach(
                    (resolution) =>
                        processResolution(
                            this,
        ...
        if (component === "**") {
            ...
            if (usage !== "exclude") {
                ...
                if (component.charCodeAt(0) === CharacterCodes.asterisk) {
                    ...
                    component.charCodeAt(0) === CharacterCodes.question
                ) {
                    componentPattern += "[^./]";
                    component = component.substr(1);
                }
...
export function matchFiles(
    ...
    for (const basePath of patterns.basePaths) {
        visitDirectory(
            basePath,
            combinePaths(currentDirectory, basePath),
            depth
        );
    }
...
export function usesExtensionsOnImports(
    { imports }: SourceFile,
    hasExtension: (text: string) => boolean = or(
        hasJSFileExtension,
        hasTSFileExtension
    )
): boolean {
...
export function getModuleSpecifierEndingPreference(
    ...
    if (!shouldAllowImportingTsExtension(compilerOptions)) {
        ...
        return usesExtensionsOnImports(sourceFile)
            ? ModuleSpecifierEnding.JsExtension
    ...
    function inferPreference() {
        ...
        const specifiers = sourceFile.imports.length
            ? sourceFile.imports.map((i) => i.text)
            : isSourceFileJS(sourceFile)
            ? getRequiresAtTopOfFile(sourceFile).map((r) => r.arguments[0].text)
            : emptyArray;
        ...
        return usesJsExtensions
            ? ModuleSpecifierEnding.JsExtension
            : ModuleSpecifierEnding.Minimal;
    }
}

function getRequiresAtTopOfFile(
    sourceFile: SourceFile
): readonly RequireOrImportCall[] {
...
export function parseValidBigInt(text: string): PseudoBigInt {
    ...
    const base10Value = parsePseudoBigInt(
        `${negative ? text.slice(1) : text}n`
    );
 ...
 */
export function isValidBigIntString(
    s: string,
    ...
    return (
        ...
        scanner.getTokenEnd() === s.length + 1 &&
...
export function setParentRecursive<T extends Node>(
    ...
    forEachChildRecursively(
        rootNode,
        isJSDocNode(rootNode)
            ? bindParentToChildIgnoringJSDoc
            : bindParentToChild
    );
    return rootNode;
    ...
    function bindParentToChild(child: Node, parent: Node) {
        return (
            bindParentToChildIgnoringJSDoc(child, parent) || bindJSDoc(child)
        );
    }
}
...
export function getContainingNodeArray(
    ...
    switch (node.kind) {
        ...
        case SyntaxKind.Decorator: {
            ...
            return canHaveDecorators(parent) ? parent.modifiers : undefined;
        }
        case SyntaxKind.HeritageClause:
    ...
    switch (parent.kind) {
        ...
        case SyntaxKind.JsxSelfClosingElement:
            return isTypeNode(node)
                ? (parent as JsxOpeningElement | JsxSelfClosingElement)
                      .typeArguments
                : undefined;
        case SyntaxKind.Block:
        case SyntaxKind.CaseClause:
        case SyntaxKind.DefaultClause:
        case SyntaxKind.ModuleBlock:
            return (parent as Block | CaseOrDefaultClause | ModuleBlock)
                .statements;
        case SyntaxKind.CaseBlock:
...
export function isThisTypeParameter(type: Type): boolean {
    return !!(
        ...
        (type as TypeParameter).isThisType
    );
}
    ...
    while (partEnd >= 0) {
        ...
        switch (state) {
            case States.BeforeNodeModules:
                if (
                    fullPath.indexOf(nodeModulesPathPart, partStart) ===
                    partStart
                ) {
...
export function getParameterTypeNode(
    ...
    return parameter.kind === SyntaxKind.JSDocParameterTag
        ? parameter.typeExpression?.type
        : parameter.type;
}
...
export function isTypeDeclaration(
    ...
    switch (node.kind) {
        ...
        case SyntaxKind.ImportSpecifier:
        case SyntaxKind.ExportSpecifier:
            return (node as ImportSpecifier | ExportSpecifier).parent.parent
                .isTypeOnly;
        default:
            return false;
    }
...
/** @internal */
export function canHaveExportModifier(
    node: Node
): node is Extract<HasModifiers, Statement> {
    return (
...
/** @internal */
export function isOptionalJSDocPropertyLikeTag(
    node: Node
...
export function canUsePropertyAccess(
    ...
    return firstChar === CharacterCodes.hash
        ? name.length > 1 &&
              isIdentifierStart(name.charCodeAt(1), languageVersion)
...
export function isJSDocOptionalParameter(node: ParameterDeclaration) {
    return (
        ...
        ((node.type && node.type.kind === SyntaxKind.JSDocOptionalType) ||
            getJSDocParameterTags(node).some(
                ({ isBracketed, typeExpression }) =>
                    isBracketed ||
                    (!!typeExpression &&
...
export function isOptionalDeclaration(declaration: Declaration): boolean {
    switch (declaration.kind) {
        case SyntaxKind.PropertyDeclaration:
        case SyntaxKind.PropertySignature:
            return !!(declaration as PropertyDeclaration | PropertySignature)
                .questionToken;
        case SyntaxKind.Parameter:
            return (
                !!(declaration as ParameterDeclaration).questionToken ||
...
export function isNonNullAccess(node: Node): node is AccessExpression {
    ...
    return (
        (kind === SyntaxKind.PropertyAccessExpression ||
            kind === SyntaxKind.ElementAccessExpression) &&
        isNonNullExpression((node as AccessExpression).expression)
    );
}

/** @internal */
export function isJSDocSatisfiesExpression(
    node: Node
): node is JSDocSatisfiesExpression {
    return (
        isInJSFile(node) &&
        isParenthesizedExpression(node) &&
        hasJSDocNodes(node) &&
        !!getJSDocSatisfiesTag(node)
    );
}

/** @internal */
export function getJSDocSatisfiesExpressionType(
    node: JSDocSatisfiesExpression
) {
...
/** @internal */
export function getEscapedTextOfJsxAttributeName(
    node: JsxAttributeName
): __String {
    return isIdentifier(node)
        ? node.escapedText
        : getEscapedTextOfJsxNamespacedName(node);
}
...
export function isJsxAttributeName(node: Node): node is JsxAttributeName {
    const kind = node.kind;
    return (
        kind === SyntaxKind.Identifier || kind === SyntaxKind.JsxNamespacedName
    );
}

/** @internal */
export function getEscapedTextOfJsxNamespacedName(
